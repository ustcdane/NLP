...
{ 
	...
	else
	{
		#pragma omp parallel
		{
			...
			#pragma omp for
			for (int n=0; n<nNode; n++)// split by threads
			{
    ...
    {
		// check convergence
	    if (fabs(mPreDistance-mCurDistance)/mPreDistance < 0.0001) break;
...
void compress_layer_weights(PyObject *pComprsLabel, PyObject *pCodeBook, PyObject *pWeights, int nWeight, int nBit)
{
	...
	kmeans_cluster(cLabel, cCentroid, cWeights, nWeight, 1, nCentroid);
